...
function DialogCreateNewWord({
   show,
   setShow,
   sectionId,
   itemData,
}: IdialogWordDataProps): JSX.Element {
   ...
   function editWord(word: IWordData) {
      setTopicList((prev) => {
         ...
         let wordIndex = findIndexOfELement(
            localList[sectionIndex].data,
            word.id
         );
   ...
   function deleteWord(event: React.MouseEvent) {
      ...
      setTopicList((prev) => {
         ...
         localList[findIndexOfELement(localList, sectionId)].data.splice(
            findIndexOfELement(
               localList[findIndexOfELement(localList, sectionId)].data,
               itemData.id
            ),
            1
         );
         ...
         >
            <div className={styles.addWordBlock}>
               <form className={styles.addWordForm} onSubmit={getAllData}>
                  ...
                  {showDropdown && (
                     <div className={styles.imageSelector}>
                        <div className={styles.imageSelectorInputs}>
                           ...
                           <CustomInput
                              placeholder="Write link to the image"
                  ...
                  <CustomBtn>{itemData.id ? "Edit" : "Add"} Word</CustomBtn>
               </form>
            </div>
         </CustomDialog>
      </>
