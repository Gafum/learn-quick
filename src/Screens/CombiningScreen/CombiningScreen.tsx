...
function CombiningScreen(): JSX.Element {
   ...
   function startTest() {
      window.scrollTo(0, 0);
      setRightSideList(shuffleArray([...myIterableList]).slice(0, 15));
      setLeftSideList(shuffleArray([...myIterableList]).slice(0, 15));
   }
   ...
   function checkAnswer(choosedElement: NumStr) {
      ...
      if (choosedElement == selectedElement) {
         ...
         setRightSideList((prev) => {
            let localList: IWordDataCombiningScreen[] = JSON.parse(
               JSON.stringify(prev)
            );
            localList[
               findIndexOfELement(localList, choosedElement)
            ].wrongAnswer = true;
            return localList;
         });
      }
   ...
   if (leftSideList.length == 0) {
      return (
         <div style={{ display: "flex", justifyContent: "center" }}>
            <CustomBtn onClick={startTest}>Repeate Test</CustomBtn>
         </div>
      );
   }
   ...
   return (
      <LazyMotion features={domAnimation}>
         <m.div {...ScreensAnimation} className={styles.combiningScreen}>
            <CombiningListShower
               list={leftSideList}
               mySide={false}
               oneBlockData={{
